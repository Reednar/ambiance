import { Controller, Get, Post, Res, Body, Req, Headers, UseGuards, UseInterceptors, Request, Param } from '@nestjs/common';
...
export class PaiementsController {
  ...
  async create(@Res() res: Response, @Body() body: any) {
    try {
      ...
      const paiement = await this.stripeService.savePaymentReceipt(sessionId, userId);
  ...
  async getSessionStatus(@Res() res: Response, @Param('sessionId') sessionId: string) {
    try {
      const session = await this.stripeService.getSessionStatus.call({stripe: this.stripe}, sessionId);
